...
public class NonCpmVp {
	...
	public void procVp(String message, Session session, WsInfoVO wsInfo) throws VerifierException {
		...
		try {
			...
			if (request == null) {
				throw new VerifierException(ProxyErrorEnum.MISSING_MANDATORY_ITEM, trxcode, "request");
			}

			if (vp == null) {
				throw new VerifierException(ProxyErrorEnum.MISSING_MANDATORY_ITEM, trxcode, "vp");
			}

			if (!trxcode.equals(msgVp.getTrxcode())) {
				throw new VerifierException(ProxyErrorEnum.TRXCODE_NOT_FOUND, trxcode);
			}

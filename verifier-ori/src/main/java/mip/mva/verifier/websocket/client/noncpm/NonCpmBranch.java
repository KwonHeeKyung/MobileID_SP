package mip.mva.verifier.websocket.client.noncpm;
...
import mip.mva.verifier.websocket.proc.noncpm.NonCpmDefaultProc;
import mip.mva.verifier.websocket.proc.noncpm.NonCpmError;
import mip.mva.verifier.websocket.proc.noncpm.NonCpmJoin;
import mip.mva.verifier.websocket.proc.noncpm.NonCpmProfile;
import mip.mva.verifier.websocket.proc.noncpm.NonCpmVerify;
import mip.mva.verifier.websocket.proc.noncpm.NonCpmVp;
import mip.mva.verifier.websocket.vo.MsgError;

/**
 * @Project     : 모바일 운전면허증 서비스 구축 사업
 * @PackageName : mip.mva.verifier.websocket.client.noncpm
...
public class NonCpmBranch {
	 ...
	 */
	public void packetChoose(String message, Session session, WsInfoVO wsInfo) throws VerifierException {
		...
		if (msg == null) {
			...
			MsgError msgError = new MsgError(trxcode, ProxyErrorEnum.MISSING_MANDATORY_ITEM.getCode(),
					ProxyErrorEnum.MISSING_MANDATORY_ITEM.getMsg());

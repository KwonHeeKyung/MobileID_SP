	...
	public Boolean verifyVP(String trxcode, VP vp) throws VerifierException {
		...
		if (vcStatus.equalsIgnoreCase(VcStatusEnum.ACTIVE.getVal())) { // 활성화 상태
			...
			if (memo.equals("주소변경")) {
				...
				throw new VerifierException(MipErrorEnum.UNKNOWN_ERROR, trxcode, "제출불가 상태 : " + vcStatus + "(" + memo + ")");
			}
		} else {
			throw new VerifierException(MipErrorEnum.UNKNOWN_ERROR, trxcode, "제출불가 상태 : " + vcStatus);
		}
		// VP 상태 확인 End
